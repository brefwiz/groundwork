...
mod tests {
    ...
    async fn envelope_crypto_seal_open_round_trip() {
        ...
        let ciphertext = crypto.seal(&kek_source, subject, plaintext).await.unwrap();

        let decrypted = crypto
            .open(&kek_source, subject, &ciphertext)
            .await
            .unwrap();
    ...
    async fn envelope_crypto_with_dyn_kek_source() {
        ...
        let decrypted = crypto.open(kek_source, subject, &ciphertext).await.unwrap();
